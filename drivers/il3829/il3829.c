...
int il3829_init(il3829_t *dev, const spi_epd_params_t *params,
...
void il3829_display_init(il3829_t *dev)
{
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_DRIVER_OUTPUT_CONTROL, (uint8_t *)y_data, 3);
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_BOOSTER_SOFT_START_CONTROL, data, 3);

    data[0] = IL3829_VCOM;
    spi_epd_write_cmd(&dev->params, IL3829_CMD_WRITE_VCOM_REGISTER, data, 1);

    data[0] = 0x1A; /* 4 dummy line per gate */
    spi_epd_write_cmd(&dev->params, IL3829_CMD_SET_DUMMY_LINE_PERIOD, data, 1);

    data[0] = 0x08; /* 2 µs per line */
    spi_epd_write_cmd(&dev->params, IL3829_CMD_SET_GATE_LINE_WIDTH, data, 1);

    data[0] = (uint8_t)dev->entry_mode;
    spi_epd_write_cmd(&dev->params, IL3829_CMD_DATA_ENTRY_MODE_SETTING, data, 1);
}
...
void il3829_activate(il3829_t *dev)
{
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_DISPLAY_UPDATE_CONTROL_2, data, sizeof data);
    spi_epd_write_cmd(&dev->params, IL3829_CMD_MASTER_ACTIVATION, NULL, 0);
    spi_epd_wait(&dev->params, IL3829_WAIT_ACTIVATION);
}
...
void il3829_init_full(il3829_t *dev)
{
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_WRITE_LUT_REGISTER,
...
void il3829_update_full(il3829_t *dev)
{
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_DISPLAY_UPDATE_CONTROL_2, data, sizeof data);
    spi_epd_write_cmd(&dev->params, IL3829_CMD_MASTER_ACTIVATION, NULL, 0);
    spi_epd_wait(&dev->params, IL3829_WAIT_UPDATE_FULL);
    spi_epd_write_cmd(&dev->params, IL3829_CMD_NOP, NULL, 0);
}
...
void il3829_update_part(il3829_t *dev)
{
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_DISPLAY_UPDATE_CONTROL_2, data, sizeof data);
    spi_epd_write_cmd(&dev->params, IL3829_CMD_MASTER_ACTIVATION, NULL, 0);
    spi_epd_wait(&dev->params, IL3829_WAIT_UPDATE_PART);
    spi_epd_write_cmd(&dev->params, IL3829_CMD_NOP, NULL, 0);
}

void il3829_write_ram(il3829_t *dev)
{
    spi_epd_write_cmd(&dev->params, IL3829_CMD_WRITE_RAM, NULL, 0);
}
...
void il3829_fill(il3829_t *dev, uint8_t x1, uint8_t x2, uint16_t y1, uint16_t y2, uint8_t color)
{
    ...
    spi_epd_cmd_start(&dev->params, IL3829_CMD_WRITE_RAM, true);
...
void il3829_set_area(il3829_t *dev, uint8_t x1, uint8_t x2, uint16_t y1, uint16_t y2)
{
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_SET_RAM_X, x_data, sizeof x_data);
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_SET_RAM_Y, (uint8_t *)y_data, sizeof y_data);

    /* Set counters to start positions */
    spi_epd_write_cmd(&dev->params, IL3829_CMD_SET_RAM_X_ADDR_COUNTER, x_data, 1);
    spi_epd_write_cmd(&dev->params, IL3829_CMD_SET_RAM_Y_ADDR_COUNTER, (uint8_t *)y_data, 2);
}

void il3829_write_buffer(il3829_t *dev, const uint8_t *buf, size_t len)
{
    spi_epd_write_cmd(&dev->params, IL3829_CMD_WRITE_RAM, buf, len);
}
...
void il3829_sleep(il3829_t *dev)
{
    ...
    spi_epd_write_cmd(&dev->params, IL3829_CMD_DEEP_SLEEP_MODE, &data, 1);
}
...
void il3829_wake(il3829_t *dev)
{
    ...
    if (dev->params.rst_pin != GPIO_UNDEF) {
        ...
        spi_epd_wait(&dev->params, IL3829_WAIT_RESET);
...
{
    spi_epd_write_cmd(&dev->params, IL3829_CMD_SWRESET, NULL, 0);
}
